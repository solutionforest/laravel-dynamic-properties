...
describe('Property Error Handling', function () {
    ...
    describe('PropertyService error handling', function () {
        ...
        it('throws PropertyValidationException for invalid values', function () {
            ...
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_text', ''))
                ->toThrow(PropertyValidationException::class);

            // Test length validation
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_text', 'ab'))
                ->toThrow(PropertyValidationException::class);

            // Test range validation
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_number', -1))
                ->toThrow(PropertyValidationException::class);

            // Test select options validation
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_select', 'invalid'))
                ->toThrow(PropertyValidationException::class);
        });
        ...
        it('throws PropertyOperationException for unsaved entity', function () {
            ...
            expect(fn () => $this->service->setProperty($unsavedEntity, 'error_test_text', 'value'))
        ...
        it('provides detailed error context in exceptions', function () {
            ...
            try {
                $service->setProperty($entity, 'non_existent', 'value');
    ...
    describe('HasProperties trait error handling', function () {
        ...
        it('throws exceptions through trait methods', function () {
            ...
            expect(fn () => $this->entity->setProperty('error_test_text', ''))
                ->toThrow(PropertyValidationException::class);
        });
    ...
    describe('Error message quality', function () {
        ...
        it('provides user-friendly error messages', function () {
            try {
                $this->service->setProperty($this->entity, 'error_test_text', 'ab');
            } catch (PropertyValidationException $e) {
        ...
        it('includes validation context in error arrays', function () {
            try {
                $this->service->setProperty($this->entity, 'error_test_text', '');
            } catch (PropertyValidationException $e) {
                $array = $e->toArray();
    ...
    describe('Edge cases and error recovery', function () {
        ...
        it('handles null and empty values correctly', function () {
            ...
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_number', null))
                ->not->toThrow(\Exception::class);

            // Required field should reject null
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_text', null))
                ->toThrow(PropertyValidationException::class);
        ...
        it('handles type conversion errors gracefully', function () {
            expect(fn () => $this->service->setProperty($this->entity, 'error_test_number', 'not a number'))
                ->toThrow(PropertyValidationException::class);
        });

        it('maintains data consistency on batch operation failures', function () {
            // Set some initial valid properties
            $this->service->setProperty($this->entity, 'error_test_text', 'valid');
            $this->service->setProperty($this->entity, 'error_test_number', 50);
            ...
            try {
                ...
                expect($this->entity->getProperty('error_test_text'))->toBe('valid');
                expect($this->entity->getProperty('error_test_number'))->toBe(50.0);

                // New property should not be set
                expect($this->entity->getProperty('error_test_select'))->toBeNull();
            }
        });
    });
});
